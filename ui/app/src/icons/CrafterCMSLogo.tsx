...
interface CrafterCMSLogoProps {
  ...
  className?: string;
  width?: number | 'auto';
  sxs?: PartialSxRecord<'root' | 'crafter' | 'redStuff'>;
}

let UND;

export default function CrafterCMSLogo(props: CrafterCMSLogoProps) {
  const { sxs, width, alt, className } = props;
  const themeSx = { fill: (theme) => (theme.palette.mode === 'dark' ? '#fff' : '#000'), ...sxs?.crafter };
  const redStuff = { fill: '#f00', ...sxs?.redStuff };
  return (
    <Box
      component="svg"
      aria-label={alt ?? ''}
