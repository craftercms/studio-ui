...
export function useExtractVideoPickerDataSources(contentType: ContentType, field: ContentTypeField): DataSource[] {
	const dataSources = contentType.dataSources;
	const dataSourceIdString = (field.properties.videoManager?.value as string) ?? '';
	return useMemo(() => {
