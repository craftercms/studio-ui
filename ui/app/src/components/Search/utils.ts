...
import { contentEvent, itemDuplicated, itemsDeleted, itemsPasted } from '../../state/actions/system';
...
export const useSearchState = ({ searchParameters, onSelect }: useSearchStateProps): useSearchStateReturn => {
  ...
  useEffect(() => {
    const eventsThatNeedReaction = [contentEvent.type, itemDuplicated.type, itemsDeleted.type, itemsPasted.type];
