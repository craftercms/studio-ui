...
// TODO: update when API is ready - right now it's a mock response
export function validateRestoreMock(packages): Observable<{
  packageId: string;
  resolutionStrategies: string[];
  conflicts: RecycleBinConflict[];
}> {
  const response = {
    packageId: '123',
    resolutionStrategies: ['suffix'],
    conflicts: [
      {
        path: "/static-assets/images/test.png'",
        resolutionStrategies: [
          {
            strategy: 'suffix',
            resolvedPath: '/static-assets/images/test-2.png'
          }
        ]
      }
    ]
...
// TODO: update when API is ready - right now it's a mock response
export function fetchRecycleBinPackagesMock(siteId: string): Observable<RecycleBinPackage[]> {
  const response: RecycleBinPackage[] = [
    {
      id: '142',
      comment: 'Delete Screenshot 2022-08-02 at 12.44.44.png',
      numberOfItems: 1,
      published: 'no',
      timestamp: '2022-08-18T13:42:48-06:00',
      user: {
        id: 0,
        username: 'admin',
        firstName: 'Admin',
...
export function fetchRecycleBinPackageMock(siteId: string, packageId: string): Observable<RecycleBinDetailedPackage> {
  return new Observable((subscriber) => {
    setTimeout(() => {
      subscriber.next({
        id: '142',
        ...
        items: [
          {
            ...
            dateCreated: '2022-08-03T11:21:11-06:00',
            modifier: 'admin',
            dateModified: '2022-08-19T15:00:45-06:00',
            commitId: '40fd1c3da2b1365cabdb53b0ec77a7b602ac994a',
            sizeInBytes: 1005,
            // staging: null,
            // live: null,
            id: 96987,
...
// TODO: update when API is ready - right now it's a mock response
export function restoreRecycleBinPackagesMock(resolution) {
  return new Observable((subscriber) => {
    setTimeout(() => {
      subscriber.next();
      subscriber.complete();
...
interface FetchRecycleBinPackagesOptions {
  usernames: string[];
  offset: number;
  limit: number;
  published: string;
  dateFrom: string;
  dateTo: string;
  sort: 'username' | 'timestamp' | 'path';
  order: 'asc' | 'desc';
  keyword: string;
}

export function fetchRecycleBinPackages(siteId: string, options?: Partial<FetchRecycleBinPackagesOptions>) {
  const qs = toQueryString({ siteId, ...options });
  return get(`/studio/api/2/recycle_bin/packages${qs}`).pipe(pluck('result', 'packages'));
}

export function fetchRecycleBinPackage(siteId: string, packageId: string) {
  const qs = toQueryString({ siteId, packageId });
  return get(`/studio/api/2/recycle_bin/package/${packageId}${qs}`);
}

export function validateRecycleBinRestore(siteId: string, packageId: string) {
  return postJSON('/studio/api/2/recycle_bin/validate_restore', {
    siteId,
    packageId
  }).pipe(pluck('result'));
}

// TODO: https://app.swaggerhub.com/apis/craftercms/studio/4.0.1.0#/recyclebin/recycle_bin_restore
// more resolutionStrategies will be added to they will be allowed to select (that's why we can send resolution
// strategy for each item).
export function recycleBinRestore(
  siteId: string,
  packageId: string,
  comment: string,
  resolutionStrategy: string
): Observable<boolean> {
  return postJSON('studio/api/2/recycle_bin/restore', {
    siteId,
    packageId,
    comment,
    resolutionStrategy
