...
import { RecycleBinPackage } from '../components/RecycleBin';
...
export function validateRestore(packages) {
  const response = {
    conflicts: [
      {
        conflictingPath: '/static-assets/images/test.png',
        resolvedPath: '/static-assets/images/test-2.png'
      },
      {
        conflictingPath: '/static-assets/images/aguacate.png',
        resolvedPath: '/static-assets/images/aguacate-2.png'
      }
    ]
  };
...
// TODO: update when API is ready - right now it's a mock response
export function fetchRecycleBinPackages(siteId: string): Observable<RecycleBinPackage[]> {
...
// TODO: update when API is ready - right now it's a mock response
export function restoreRecycleBinPackages(ids: string[]) {
  return new Observable((subscriber) => {
    setTimeout(() => {
      subscriber.next();
      subscriber.complete();
