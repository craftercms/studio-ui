...
const trapFocus = require('@uppy/dashboard/lib/utils/trapFocus');
const cuid = require('cuid');
const ResizeObserver = require('resize-observer-polyfill').default || require('resize-observer-polyfill');
const createSuperFocus = require('@uppy/dashboard/lib/utils/createSuperFocus');
...
class Dashboard extends Plugin {
  ...
  hideAllPanels = () => {
    ...
    if (
      current.activePickerPanel === update.activePickerPanel &&
  ...
  closeModal = (opts = {}) => {
    ...
    if (manualClose) {
      if (this.opts.browserBackButtonClose) {
        ...
        if (window.history.state && window.history.state[this.modalName]) {
  ...
  _getProgressIndicators = memoize((targets) => {
    return targets.filter((target) => target.type === 'progressindicator').map(this._attachRenderFunctionToTarget);
  });

  _getEditors = memoize((targets) => {
    return targets.filter((target) => target.type === 'editor').map(this._attachRenderFunctionToTarget);
  });
