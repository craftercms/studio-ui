...
CStudioAuthoring.Module.requireModule(
	...
	{ moduleLoaded: function () {
		...
		var moduleName = "dam-panel",		// ties css file to the js file
		modulePath = null,
		YDom = YAHOO.util.Dom,
	...
	CStudioAuthoring.DamPanel = CStudioAuthoring.DamPanel || {
		...
		initialize: function(config) {
			...
			if(this.initialized == false) {
				...
				amplify.subscribe('/dam/operation/started', function () {
					self.ajaxOverlay.show();
				});
				amplify.subscribe('/dam/operation/completed', function () {
					self.ajaxOverlay.hide();
				});
				amplify.subscribe('/dam/operation/failed', function () {
					self.ajaxOverlay.hide();
				});
				
				amplify.subscribe('/dam/page-model/loaded', function (data) {
				...
				amplify.subscribe('components/form-def/loaded', function (data) {
					if(data.isNew && data.isNew == true) {
						amplify.subscribe('/dam/operation/completed', function (data) {
			...
			CStudioAuthoring.Service.lookupConfigurtion(
					...
					"/preview-tools/components-config.xml", 
					{
						success: function(config) {
							...
							self.renderComponents(containerEl, config, loadImagesCb);
						},
		...
		collapse: function(containerEl, config) {
			...
			if(this.dcOverlay) {
				...
				var context = CStudioAuthoring.DamPanel;
		...
		createMenuComponent : function (component, tipText) {
			...
			component.type = "/component/image";
			component.label = component.internalName;
			component.path = component.browserUri;

			if (component && component.path && component.type && component.label) {
				YDom.addClass(menuComponent, "acn-panel-component");
				
				menuComponent.innerHTML = '<div style="width:200px; float:left;" class="acn-panel-dam ' + dcNewComponentClass + ' ' + dcComponentClass + 
												'" data-path="' + component.path + '" data-type="' + component.type +
												'"><div><img width="200px" src="'+component.browserUri+'"/></div></div>';
			}
			return menuComponent;
		},

		renderComponents: function(containerEl, config, renderDoneCb) {
			containerEl.innerHTML = "";
			var realContainerEl = containerEl;

			var containerEl = document.createElement("div");
			containerEl.style = "height: 125px;  width: 100%; overflow-y: scroll;"
        	realContainerEl.appendChild(containerEl);


			CStudioAuthoring.Service.lookupSiteContent(CStudioAuthoringContext.site, 
				...
				"default", 
				{
	            	success: function(treeData) {
						...
						for(var j=0; j<items.length; j++) {
							var item = items[j];
							if(item.browserUri.indexOf(".")!= -1) {
								var componentEl = this.createMenuComponent(item, "Adding new component:");
								this.containerEl.appendChild(componentEl);
							}
						}

						this.renderDoneCb.imagesLoaded();
	            	},
	            	failure: function() {
	            	},
	            	createMenuComponent: this.createMenuComponent,
	            	containerEl: containerEl,
	            	renderDoneCb: renderDoneCb
	...
	DragAndDropDecorator = function(id, sGroup, config) {
	    ...
	    var zIndexRef = (CStudioAuthoring.DamPanel.dcOverlay) ? CStudioAuthoring.DamPanel.dcOverlay.getzIndex() : 0;
	...
	YAHOO.extend(DragAndDropDecorator, YAHOO.util.DDProxy, {
	    ...
	    startDrag: function(x, y) {
	        ...
	        if (!this.dragging) {
	        	this.dragging = true;
		        	...
		        	zIndexRef = CStudioAuthoring.DamPanel.getMaxzIndex(srcEl, 1);

		        	YDom.setStyle(proxy, "z-index", zIndexRef + 2);
		        	proxy.style.background = CStudioAuthoring.DamPanel.getInheritedBackground(srcEl);
	            ...
	            YDom.setStyle( proxy, "height", "auto" );
		        	...
		        	zIndexRef = (CStudioAuthoring.DamPanel.dcOverlay) ? CStudioAuthoring.DamPanel.dcOverlay.getzIndex() : 0;
				    YDom.setStyle(proxy, "z-index", zIndexRef + 2);
	    ...
	    onInvalidDrop : function (e) {
	        ...
	        this.dragging = false;
			Utility.resizeProxy(srcEl, proxy);
			CStudioAuthoring.DamPanel.clearActive();
		...
		onDragDrop: function(e, id) {
			...
			if(!YDom.hasClass(srcEl, "acn-panel-dam")) {
				return;
			}
				...
				// var destDD = YAHOO.util.DragDropMgr.getDDById(id);
		 		...
		 		if (YDom.hasClass(srcEl, dcNewComponentClass)) {
		 			...
		 			var componentPath = srcEl.getAttribute('data-path'),
		 				...
		 				imageFileName = CStudioAuthoring.Utils.generateUUID()+".xml",
		 				imageComponentName = "/site/components/imgs/"+imageFileName,
		 				cpl;
					...
					var saveSvcCb = {
						success:function() {
							...
							srcEl.componentPlaceholder.modelData = { key: imageComponentName, value: "Image", include: imageComponentName, };	
		 		 			cpl.modelData = { key: imageComponentName, value: "Image", include: imageComponentName };
					 		CStudioAuthoring.DamPanel.getPageModel(CStudioAuthoring.DamPanel.getPreviewPagePath(CStudioAuthoringContext.previewCurrentPath), "save-components-new", true, false);
						},
						failure: function() {
						},
						cpl: cpl,
					...
					var value = "<component>" +
								  ...
								  "<display-template>/templates/web/image-component.ftl</display-template>" +
								  "<merge-strategy>inherit-levels</merge-strategy>" +
								  //"<objectGroupId>285f</objectGroupId>" +
								  //"<objectId>285f3263-249c-a02d-5f61-a2d3d28990c2</objectId>" +
								  "<folder-name>imgs</folder-name>" +
								  "<file-name>"+imageFileName+"</file-name>" +
								  "<image>"+componentPath+"</image>" +
								  "<alttext/>" +
								"</component>";

					var writeServiceUrl = "/proxy/alfresco/cstudio/wcm/content/write-content"
					                    + "?site=" + CStudioAuthoringContext.site 
					                    + "&path=" + imageComponentName
					                    + "&fileName=" + imageFileName 
				                        + "&contentType=/component/image" 
   				                        + "&createFolders=true"  
 				                  		+ "&unlock=true";
	    ...
	    onDragEnter: function(e, id) {
			...
			if (!srcContainer && YDom.hasClass(srcEl, dcNewComponentClass)) {
				...
				if (destContainer) {
					...
					srcEl.componentPlaceholder = CStudioAuthoring.DamPanel.createNewComponentPlaceholder(srcEl);
	...
	CStudioAuthoring.Module.moduleLoaded("dam-panel", CStudioAuthoring.DamPanel);
