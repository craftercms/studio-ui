  ...
  (CStudioSearch.init = function () {
    CStudioSearch.loadFiltersAndResultTemplates({
      success: function () {
        ...
        var keywordEnterKeyListener = new YAHOO.util.KeyListener(
          keywordTextBox,
          { keys: 13 },
          searchCallback,
          'keydown'
        ...
        var paginationEnterKeyListener = new YAHOO.util.KeyListener(
          paginationTextBox,
          { keys: 13 },
          searchCallback,
          'keydown'
...
CStudioSearch.determineFilterRendererFromUrl = function () {
  ...
  if (!filterRenderer) {
    ...
    CStudioAuthoring.Module.requireModule(paramContext, modulePath, moduleConfig, moduleCb);
  }
...
CStudioSearch.initializeSearchFilter = function (filter) {
  ...
  if (sortOptions.length > 0) {
    for (var i = 0; i < sortOptions.length; i++) {
      ...
      if (curOption.value == CStudioSearch.searchContext.sortBy) sortBySelectEl.options[i].selected = true;
    }
  } else {
...
CStudioSearch.updateSearchContextWithBaseOptions = function () {
  ...
  CStudioSearch.searchContext.keywords = encodeURIComponent(
    document.getElementById('cstudio-wcm-search-keyword-textbox').value
  );
...
(CStudioSearch.pathToId = function (path) {
  ...
  (CStudioSearch.renderCommonResultWrapper = function (contentTO, resultBody) {
    return (
      "<div class='cstudio-search-result'>" +
      "<div id='result-select-" +
      contentTO.resultId +
      "' class='cstudio-search-select-container'></div>" +
      "<div class='result-status' id='result-status" +
      contentTO.resultId +
      "' style='float: left !important; margin-left: 15px;'></div>" +
...
CStudioSearch.renderPagination = function (resultCount, pageCount, searchContext, searchFailed, failCause) {
  ...
  if (resultCount == 0) {
    ...
    if (endItem > resultCount) endItem = resultCount;
  ...
  if (previousDisabled) {
    // this can be done by disbaling the link+ccs right?
  ...
  var startPage =
    parseInt(CStudioSearch.searchContext.currentPage) >= 5 ? parseInt(CStudioSearch.searchContext.currentPage) - 5 : 0;
...
CStudioSearch.fireSearchRequest = function (searchContext) {
  ...
  var callback = {
    success: function (results) {
      ...
      if (resultTableEl && results) {
        ...
        for (var i = 0; i < results.objectList.length; i++) {
          ...
          if (searReultItem) {
            var itemTitle = CStudioAuthoring.Utils.buildToolTip(
              contentItem.item.internalName,
              contentItem.item.internalName,
              contentItem.item.contentType
            );
    ...
    failure: function (o) {
      ...
      YDom.get('cstudio-wcm-search-result').innerHTML =
        "<p align='center'><strong>" + CMgs.format(langBundle, 'errSearchFailed', '') + '</strong></i></p>';
    },
...
CStudioSearch.loadFiltersAndResultTemplates = function (callback) {
  var triggerOnCompleteCb = {
    ...
    config = {
      resultTemplates: { template: { name: 'default', type: '*' } },
      filters: { filter: { default: 'true', name: 'default', label: 'Default' } }
    };
