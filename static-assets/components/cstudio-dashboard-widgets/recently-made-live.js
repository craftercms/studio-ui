  ...
  this.renderItemsHeading = function () {
    ...
    var header =
      ...
      WcmDashboardWidgetCommon.getSimpleRow(
        'endpoint',
        widgetId,
        CMgs.format(langBundle, 'dashletRecentDeployColEndpoint'),
        'minimize'
      ) +
  ...
  this.renderLineItem = function (item, isFirst, count) {
    if (!item.folder) {
      var html = [],
        ...
        displayName = WcmDashboardWidgetCommon.getFormattedString(CStudioAuthoring.Utils.escapeHTML(name), 40),
        editLinkId;
      ...
      if (isFirst) {
        ...
        var browserUri = CStudioAuthoring.Operations.getPreviewUrl(item, false, true),
          displayBrowserUri = CStudioAuthoring.Utils.escapeHTML(
            WcmDashboardWidgetCommon.getFormattedString(browserUri, 80)
          ),
          uri = CStudioAuthoring.Utils.escapeHTML(item.uri);
