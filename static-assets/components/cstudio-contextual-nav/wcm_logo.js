...
CStudioAuthoring.ContextualNav.WcmLogo = CStudioAuthoring.ContextualNav.WcmLogo || {
  ...
  initialize: function (config) {
    YAHOO.util.Event.onAvailable(
      'acn-wcm-logo',
      function () {
        YDom.get('acn-wcm-logo-image').src =
          CStudioAuthoringContext.baseUri +
          '/api/1/services/api/1/content/get-content-at-path.bin?path=/cstudio/config/app-logo.png';
        ...
        CStudioAuthoring.Service.lookupSiteLogo(CStudioAuthoringContext.site, {
          success: function (response) {
            ...
            if (response) {
              YDom.get('cstudio-logo').src =
                '/studio/api/1/services/api/1/server/get-ui-resource-override.json?resource=logo.jpg';
            } else {
