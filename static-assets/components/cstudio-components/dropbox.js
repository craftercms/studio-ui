...
(function () {
    ...
    drop: function (e) {
      ...
      if (!document.getElementById('folder' + cfg.formData.path)) {
        display.insertAdjacentHTML(
          'beforeEnd',
          '<div class="folder-container" data-path="' +
            cfg.formData.path +
            '" id="folder' +
            cfg.formData.path +
            '"></div>'
      ...
      function traverseFileTree(item, cfg) {
        ...
        if (item.isFile) {
          ...
          var serviceUri = CStudioAuthoring.Service.createServiceUri(
            '/api/1/services/api/1/content/create-folder.json?site=' +
              cfg.site +
              '&path=' +
              cfg.path +
              '&name=' +
      ...
      for (var i = 0; i < items.length; i++) {
        try {
          //chrome feature to upload folders
  ...
  DP.upload = function (fileID, cfg) {
    ...
    xhr.upload.addEventListener(
      ...
      function (e) {
        if (e.lengthComputable) {
          me.showUploadProgress(elemProgress, Math.round((e.loaded * 100) / e.total), fileUI);
        }
      },
      false
    );
    ...
    xhr.addEventListener('load', function (e) {
      ...
      _.fire.call(me, success ? SUCCESS : ERROR, { ui: fileUI, file: file });
    });
