...
CStudioAuthoring.Dialogs.DialogSelectContentType = CStudioAuthoring.Dialogs.DialogSelectContentType || {
  ...
  createDialog: function (path, selectTemplateCb) {
    ...
    newdiv.innerHTML =
      '<div class="contentTypePopupInner" id="ct_contentTypePopupInner" style="width:600px;height:501px;">' +
      ...
      CMgs.format(formsLangBundle, 'chooseContentType') +
      '</div> ' +
      '<div>' +
      CMgs.format(formsLangBundle, 'chooseContentTypeBody') +
      '</div> ' +
      ...
      '</div><select id="wcm-content-types-dropdown" size="16" class="cstudio-wcm-popup-select-control" style="width:273px; height:275px;"></select> ' +
      '</div></div>' +
      ...
      CMgs.format(formsLangBundle, 'ok') +
      '" />' +
      '<input type="submit" class="btn btn-default cstudio-xform-button cancel" id="closeWCMPopup" value="' +
    ...
    var resizeHandler = new YAHOO.util.Resize('ct_contentTypePopupInner', {
      proxy: true,
      minHeight: 440,
      minWidth: 600,
      handles: ['br']
