  ...
  content: function (panelId, orderJson, config) {
    ...
    for (var i = 0; i <= orderLen; ++i) {
      ...
      orderDetails.internalName = orderJson.order[i].internalName
        ? orderJson.order[i].internalName
        : orderJson.order[i].name;
      ...
      if (thisPageObject.id == orderDetails.id) {
        if (thisPageObject.order) orderNum = parseFloat(thisPageObject.order);
        else orderNum = parseFloat(orderJson.order[i].order);
      } else {
    ...
    var onDnDsubmitClick = function () {
      ...
      CStudioAuthoring.Service.reorderServiceRequest(
...
YAHOO.extend(CStudioForms.DDList, YAHOO.util.DDProxy, {
  ...
  endDrag: function (e) {
    ...
    if (!prevSibling) reorderPageMap['prevPath'] = null;
    else reorderPageMap['prevPath'] = YAHOO.util.Dom.getPreviousSibling(srcEl).id;

    var nextSibling = YAHOO.util.Dom.getNextSibling(srcEl);
    if (!nextSibling) reorderPageMap['nextPath'] = null;
    else reorderPageMap['nextPath'] = YAHOO.util.Dom.getNextSibling(srcEl).id;
