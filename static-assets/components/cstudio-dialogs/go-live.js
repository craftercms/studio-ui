 ...
 * GoLive Constructor
 */
CStudioAuthoring.Dialogs.DialogGoLive =
  CStudioAuthoring.Dialogs.DialogGoLive ||
  function () {
    ...
    moduleLoaded: function (moduleName, parentClass) {
      ...
      function setDisabled(isDisabled) {
        ...
        if (isDisabled === ENABLED) {
          ...
          if (!now && !isSchedulingOptionsReady()) {
            isDisabled = DISABLED;
      ...
      DialogPrototype.invokeGoLiveService = function () {
        ...
        if (this.removeUncheckedItemsFromJson() == -1) {
          // no items selected
          return;
        }
        ...
        if (this.selectedJsonObj.items.length != 0) {
                    ...
                    if (config.onGoLive) {
                      ...
                      for (var l = 0; l < config.onGoLive.assetGroups.length; l++) {
                        ...
                        for (var j = 0; j < this.self.itemArray.length; j++) {
                          ...
                          if (true) {
                            //compare path with wfAssetGroupConfig.include pattern
                            ...
                            createWorkflowRequest.jobs[createWorkflowRequest.jobs.length] = {
                              ...
                              properties: [
                                { name: 'submitter', value: CSAContext.user },
                                {
                                  ...
                                  value: self.selectedJsonObj.publishChannel
                                },
                                {
                                  name: 'scheduledDate',
                                  value: self.selectedJsonObj.scheduledDate
                                },
                                {
                                  name: 'submissionComment',
                                  value: self.selectedJsonObj.submissionComment
                                }
                              ]
                            };
                          } else {
          ...
          YConnect.asyncRequest(
            'POST',
            CSAContext.baseUri + '/service/ui/workflow-actions/go-live?site=' + CSAContext.site,
            serviceCallback,
            jsonSubmitString
          );
        } else {
            ...
            dependencyUrl,
            {
              success: function (o) {
                ...
                if (testRoot.querySelector('#go-pub-channel').children.length) {
                  ...
                  CSA.Utils.initCursorPosition('timepicker', [
                    'click',
                    'keydown',
                    'keyup',
                    'keypress',
                  ...
                  YDom.get('acnScrollBoxDiv').innerHTML = CSA.StringUtils.format(
                    ...
                    CSAContext.authoringAppBaseUri,
                    'Loading items &amp; dependencies, please wait.'
                  );
        ...
        YConnect.asyncRequest(
          ...
          dependencyUrl,
          {
            success: function (o) {
              ...
              if (me.dependencyJsonObj.items.length == 1) {
                // only 1 item in the json obj
                if (me.dependencyJsonObj.items[0].numOfChildren == 0) {
                  // and no children
      ...
      DialogPrototype.onDone = function () {
        ...
        if (
          (dateValue == 'Date...' || timeValue == 'Time...' || timeValue == '') &&
