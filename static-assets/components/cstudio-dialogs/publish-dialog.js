 ...
 * Submit to go live
 */
CStudioAuthoring.Dialogs.PublishDialog =
  CStudioAuthoring.Dialogs.PublishDialog ||
  function () {
...
CStudioAuthoring.Dialogs.PublishDialog.prototype.getTimeInJsonObject = function (jsonArray, browserUri) {
  for (var i = 0; i < jsonArray.length; ++i) {
    if (jsonArray[i].browserUri == browserUri) {
      if (
        (jsonArray[i].scheduled == true || jsonArray[i].now == false) &&
      ...
      for (var j = 0; j < jsonArrayChildrenLen; ++j) {
        if (jsonArray[i].children[j].browserUri == browserUri) {
          if (
            (jsonArray[i].children[j].scheduled == true || jsonArray[i].children[j].now == false) &&
    ...
    if (scheduledDate == 'now') {
      ...
      if (shedSplit.length == 2) {
        ...
        if (dateArray.length == 3) {
          datePick.value =
            (isNaN(dateArray[1]) ? dateArray[1] : parseInt(dateArray[1], 10)) +
            '/' +
            (isNaN(dateArray[2]) ? dateArray[2] : parseInt(dateArray[2], 10)) +
            '/' +
            dateArray[0];
        }
...
CStudioAuthoring.Dialogs.PublishDialog.prototype.getScheduledDateTimeForJson = function (dateValue, timeValue) {
  ...
  var scheduledDate =
    ...
    schedDate.getHours() +
    ':' +
    schedDate.getMinutes() +
    ':' +
    schedDate.getSeconds();
...
CStudioAuthoring.Dialogs.PublishDialog.prototype.updateUncheckedItemList = function (matchedElement, isChecked) {
  ...
  if (isChecked == false) {
    ...
    if (this.ifExistsInUncheckedItemsArray(url) == -1) {
...
CStudioAuthoring.Dialogs.PublishDialog.prototype.changeToNow = function () {
  var items = YDom.getElementsBy(
    function (el) {
      return el.getAttribute('title') == 'scheduledDate';
    },
    'a',
    YDom.getElementsByClassName('liveTable')[0]
  );
...
CStudioAuthoring.Dialogs.PublishDialog.prototype.checkParentSchedule = function (
  items,
  browserUri,
  jsonSchedTime,
  ...
  if (isChildNodes) {
    ...
    } else if (itemJson && itemJson.newFile && itemJson.children && itemJson.children.length >= 1) {
      parentPageIntName = itemJson.internalName;
    ...
    if (isSubmittedForDeletion) {
      if (multipleChilds) {
        CStudioAuthoring.Operations.showSimpleDialog(
          ...
          'The parent page (' +
            parentPageIntName +
            ') cannot be deleted before the children pages below:' +
            childPageIntName,
          null, //default button
          YAHOO.widget.SimpleDialog.ICON_BLOCK,
          'studioDialog'
        ...
        CStudioAuthoring.Operations.showSimpleDialog(
          ...
          'The parent page (' +
            parentPageIntName +
            ') cannot be deleted before the child page (' +
            childPageIntName +
            ').',
          null, //default button
          YAHOO.widget.SimpleDialog.ICON_BLOCK,
      ...
      if (childPageIntName != '') {
        if (multipleChilds) {
          errorMsg =
            'The children below cannot Go Live before the parent page (' +
            parentPageIntName +
            ') goes live.' +
            childPageIntName;
        } else {
          errorMsg =
            'The child page (' +
            childPageIntName +
            ') cannot Go Live before the parent page (' +
            parentPageIntName +
            ').';
        }
      }
      ...
      if (referencePageIntName != '') {
        ...
        if (multipleReferenceChilds) {
          ...
          errorMsg +=
            'Page (' +
            parentPageIntName +
            ') cannot Go Live without the reference page (' +
            referencePageIntName +
            ').';
        }
      }
...
CStudioAuthoring.Dialogs.PublishDialog.prototype.publishingChannelsInit = function () {
  ...
  var updateMsgLimit = function (event, obj) {
    ...
    if (numChars <= TWITTER_MSG_LIMIT) {
      ...
      obj.counterTextEl.innerHTML =
        ' characters over the maximum. Any characters over the maximum will not be posted in Twitter';
    }
  };
  ...
  commonAncestor = YDom.getElementsBy(
    function (el) {
      return YDom.hasClass(el, 'pub-status');
    },
    'div',
  ...
  twitterCheckBox = YDom.getElementsBy(
    function (el) {
  ...
  counterContainer = YDom.getElementsBy(
    function (el) {
