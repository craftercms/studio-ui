...
CStudioSearch.FilterRenderer.Common.PathSelect = function (rootPath) {
  ...
  (column.initializeContentTree = function (treeEl, instance) {
    ...
    CStudioAuthoring.Service.lookupSiteFolders(
      site,
      rootPath,
      1,
      'default',
    ...
    (column._drawTree = function (treeItems, tree, pathToOpenTo, instance) {
      ...
      //try{
      Lang = YAHOO.lang.escapeHTML = function (x) {
        return x;
      };
      tree.draw();
      //}catch(err) {
  ...
  column.onTriggerContextMenu = function (tree, contexMenu) {
    ...
    var menuItems = {
      write: [{ text: 'Upload', onclick: { fn: Self.uploadContent, obj: tree } }]
    };
    ...
    var permsCallback = {
      success: function (response) {
        ...
        if (isWrite != false) {
          this._self._renderContextMenu(target, contexMenu, this.component, menuItems.write, oCurrentTextNode, true);
