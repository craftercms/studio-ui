...
CStudioAdminConsole.Tool.DeploymentTools =
  CStudioAdminConsole.Tool.DeploymentTools ||
  function(config, el) {
    this.containerEl = el;
    this.config = config;
...
YAHOO.extend(CStudioAdminConsole.Tool.DeploymentTools, CStudioAdminConsole.Tool, {
  renderWorkarea: function() {
    ...
    workareaEl.innerHTML = "<div id='deployment-tool-area'>" + '</div>';
  ...
  renderQueueList: function() {
    var actions = [
      {
        name: CMgs.format(formsLangBundle, 'setQueueDialogCancelDeployment'),
        context: this,
        method: this.cancelDeployment
      }
    ];
    CStudioAuthoring.ContextualNav.AdminConsoleNav.initActions(actions);
  ...
  renderDeploymentQueue: function() {
    ...
    queueListEl.innerHTML =
      "<div class='cs-statelist'><button id='btnCancelTop' type='button' >" +
      CMgs.format(formsLangBundle, 'setQueueDialogCancelDeployment') +
      '</button>' +
      "<table id='queueTable'>" +
      '<tr>' +
      "<th class='cs-statelist-heading'><a href='#' onclick='CStudioAdminConsole.Tool.DeploymentTools.selectAll(); return false;'>" +
      CMgs.format(langBundle, 'setQueueTabSelectAll') +
      '</a></th>' +
      "<th class='cs-statelist-heading'>" +
      CMgs.format(langBundle, 'setQueueTabID') +
      '</th>' +
      "<th class='cs-statelist-heading'>" +
      CMgs.format(langBundle, 'setQueueTabPath') +
      '</th>' +
      "<th class='cs-statelist-heading'>" +
      CMgs.format(langBundle, 'setQueueTabEnvironment') +
      '</th>' +
      "<th class='cs-statelist-heading'>" +
      CMgs.format(langBundle, 'setQueueTabUser') +
      '</th>' +
      "<th class='cs-statelist-heading'>" +
      CMgs.format(langBundle, 'setQueueTabState') +
      '</th>' +
      "<th class='cs-statelist-heading'>" +
      CMgs.format(langBundle, 'setQueueTabAction') +
      '</th>' +
      "<th class='cs-statelist-heading'>" +
      CMgs.format(langBundle, 'setQueueTabScheduledDate') +
      '</th>' +
      '</tr>' +
      "</table><button type='button' id='btnCancelBot'>" +
      CMgs.format(formsLangBundle, 'setQueueDialogCancelDeployment') +
      '</button></div>';
  ...
  renderDeploymentEndpoints: function() {
    ...
    containerEl.innerHTML =
      ...
      " <option value='' >" +
      CMgs.format(formsLangBundle, 'deploymentTabSelectEndpoint') +
      '</option>' +
      '</select></div>' +
    ...
    itemSelectEl.onchange = function() {
      ...
      if (selectedIndex != 0) {
        endpointDetailsEl.innerHTML =
          ...
          "<th class='cs-statelist-heading'>" +
          CMgs.format(formsLangBundle, 'deploymentTabEndpointProperty') +
          '</th>' +
          "<th class='cs-statelist-heading'>" +
          CMgs.format(formsLangBundle, 'deploymentTabEndpointValue') +
          '</th>' +
          '</tr>' +
          '</table>';
        ...
        endpointSyncQueuEl.innerHTML =
          ...
          '<tr>' +
          "<th class='cs-statelist-heading'><a href='#' onclick='CStudioAdminConsole.Tool.DeploymentQueues.selectAll(); return false;'>" +
          CMgs.format(langBundle, 'setQueueTabSelectAll') +
          '</a></th>' +
          "<th class='cs-statelist-heading'>" +
          CMgs.format(langBundle, 'setQueueTabID') +
          ...
          '</th>' +
          "<th class='cs-statelist-heading'>" +
          CMgs.format(langBundle, 'setQueueTabOldPath') +
          '</th>' +
          "<th class='cs-statelist-heading'>" +
          CMgs.format(langBundle, 'setQueueTabEnvironment') +
          '</th>' +
          ...
          '</th>' +
          "<th class='cs-statelist-heading'>" +
          CMgs.format(langBundle, 'setQueueTabVersion') +
          '</th>' +
          "<th class='cs-statelist-heading'>" +
          CMgs.format(langBundle, 'setQueueTabAction') +
          '</th>' +
          "<th class='cs-statelist-heading'>" +
          CMgs.format(langBundle, 'setQueueTabContentTypeClass') +
          '</th>' +
          '</tr>' +
          '</table>';
        ...
        var getVersionCb = {
          success: function(response) {
            ...
            var serviceUri =
              '/api/1/services/api/1/deployment/get-sync-target-queue.json?site=' +
              CStudioAuthoringContext.site +
              '&endpoint=' +
              endpoint.name +
              '&targetVersion=1'; //+versionResponse;
  ...
  cancelDeployment: function() {
    ...
    for (var i = 0; i < list.length; i++) {
      ...
      var serviceUri =
        '/api/1/services/api/1/deployment/cancel-deployment.json?site=' +
        CStudioAuthoringContext.site +
        '&path=' +
        path +
        '&deploymentId=' +
  ...
  renderDeploymentJobs: function() {
    var containerEl = document.getElementById('deployment-tool-area');
    containerEl.innerHTML = "<div id='jobsDetails' class='deployment-jobs-window'>" + '</div>';
...
CStudioAuthoring.Module.moduleLoaded(
  'cstudio-console-tools-deployment-tools',
  CStudioAdminConsole.Tool.DeploymentTools
);
