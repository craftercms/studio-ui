...
YAHOO.extend(CStudioAdminConsole.Tool.Logging, CStudioAdminConsole.Tool, {
  renderWorkarea: function () {
    ...
    workareaEl.innerHTML = "<div id='logger-list'>" + '</div>';
    ...
    cb = {
      success: function (response) {
        ...
        var tableHTML =
          ...
          '<tr>' +
          "<th class='cs-loggerlist-heading'>" +
          CMgs.format(langBundle, 'logginLevelsTabLogger') +
          '</th>' +
          "<th class='cs-loggerlist-heading'>" +
          CMgs.format(langBundle, 'logginLevelsTabCurrentLevel') +
          '</th>' +
          "<th class='cs-loggerlist-heading'>" +
          CMgs.format(langBundle, 'logginLevelsTabChangeLevel') +
          '</th>' +
          '</tr>';
        ...
        for (var i = 0; i < loggers.length; i++) {
          ...
          var rowHTML =
            ...
            '</td>' +
            "<td class='cs-loggerlist-detail'>" +
            CMgs.format(langBundle, logger.level) +
            '</td>' +
            "<td class='cs-loggerlist-detail'>" +
            '<a onclick="CStudioAdminConsole.Tool.Logging.prototype.setLoggingLevel(' +
            i +
            ",'debug'); return false;\">" +
            CMgs.format(langBundle, 'debug') +
            '</a>&nbsp;&nbsp;' +
            '<a onclick="CStudioAdminConsole.Tool.Logging.prototype.setLoggingLevel(' +
            i +
            ",'warn'); return false;\">" +
            CMgs.format(langBundle, 'warn') +
            '</a>&nbsp;&nbsp;' +
            '<a onclick="CStudioAdminConsole.Tool.Logging.prototype.setLoggingLevel(' +
            i +
            ",'info'); return false;\">" +
            CMgs.format(langBundle, 'info') +
            '</a>&nbsp;&nbsp;' +
            '<a onclick="CStudioAdminConsole.Tool.Logging.prototype.setLoggingLevel(' +
            i +
            ",'error'); return false;\">" +
            CMgs.format(langBundle, 'error') +
            '</a>' +
            '</td>' +
            '</tr>';
