    ...
    moduleLoaded: function () {
      ...
      YAHOO.extend(CStudioAdminConsole.Tool.ContentTypes, CStudioAdminConsole.Tool, {
        renderWorkarea: function () {
          ...
          workareaEl.innerHTML =
            "<div id='content-type-canvas'>" + '</div>' + "<div id='content-type-tools'>" + '</div>';
        ...
        titleNameValidation: function (formDef) {
          ...
          for (var i = 0; i < sections.length; i++) {
            for (var j = 0; j < sections[i].fields.length; j++) {
              ...
              if (
                (!sections[i].fields[j].id || sections[i].fields[j].id == '') &&
                sections[i].fields[j].title &&
                sections[i].fields[j].title != ''
              ) {
                              ...
                              var cb = {
                                success: function () {
                                  ...
                                  YAHOO.util.Connect.initHeader(
                                    ...
                                    CStudioAuthoring.Storage.getRequestForgeryToken()
                                  );
                          ...
                          if (validation.idError.length > 0) {
                            ...
                            if (!istemplate.flagTemplateError) {
                              ...
                              var dialog = new YAHOO.widget.SimpleDialog('errTemplates', {
                                ...
                                close: false,
                                modal: true,
                                text: CMgs.format(formsLangBundle, 'noTemplateAssocAdm'),
                                icon: YAHOO.widget.SimpleDialog.ICON_WARN,
                                constraintoviewport: true,
                                buttons: [
                                  {
                                    text: CMgs.format(formsLangBundle, 'continueEditing'),
                                    handler: function () {
                                      this.hide();
                                    },
                                    isDefault: false
                                  ...
                                  {
                                    text: CMgs.format(formsLangBundle, 'save'),
                                    handler: function () {
                                      this.hide();
                                      saveFn();
                                    },
        ...
        renderContentTypeTools: function (config) {
          ...
          toolbarEl.innerHTML =
            ...
            '<h4 id="properties-tools-panel"><span class="content-type-tools-panel-icon ttClose "></span>' +
            CMgs.format(langBundle, 'propertiesExplorer') +
            '</h4>' +
            "<div id='properties-container'></div>" +
            '</div>' +
            "<div id='basicContentContainer'><a id='basicContent'>" +
            CMgs.format(langBundle, 'basicContentTypeProp') +
            '</a></div>' +
            "<div class='content-type-tools-panel'>" +
            '<h4 id="control-tools-panel"><span class="content-type-tools-panel-icon ttClose "></span>' +
            CMgs.format(langBundle, 'controls') +
            '</h4>' +
            "<div id='widgets-container'></div>" +
            '</div>' +
            "<div class='content-type-tools-panel'>" +
            '<h4 id="datasources-tools-panel"><span class="content-type-tools-panel-icon ttClose"></span>' +
          ...
          repeatContainerEl.prototypeField = {
            ...
            getSupportedProperties: function () {
              return [
                { label: CMgs.format(langBundle, 'minOccurs'), name: 'minOccurs', type: 'string', defaultValue: '0' },
        ...
        renderDatasourcePropertySheet: function (item, sheetEl) {
          ...
          for (var i = 0; i < properties.length; i++) {
            ...
            if (itemProperty !== null) {
              ...
              item.properties[item.properties.length] = { name: property.name, value: value, type: property.type };
            }
      ...
      CStudioAdminConsole.Tool.ContentTypes.FormDefMain = {
        ...
        serializeConfigToXml: function (config, formDef) {
          ...
          xml +=
            '\t<label>' +
            CStudioForms.Util.escapeXml(config.label) +
            '</label>\r\n' +
            '\t<form>' +
            CStudioForms.Util.escapeXml(config.form) +
            '</form>\r\n' +
            '\t<form-path>' +
          ...
          if (config.paths) {
            ...
            if (config.paths.excludes) {
              ...
              if (config.paths.excludes.pattern instanceof Array) {
                ...
                xml +=
                  '\t\t\t<pattern>' + CStudioForms.Util.escapeXml(config.paths.excludes.pattern) + '</pattern>\r\n';
              }
            ...
            if (config.paths.includes) {
              ...
              if (config.paths.includes.pattern instanceof Array) {
                ...
                xml +=
                  '\t\t\t<pattern>' + CStudioForms.Util.escapeXml(config.paths.includes.pattern) + '</pattern>\r\n';
              }
