    ...
    store.prototype = {
      ...
      addDataPoint: function(x, y) {
        ...
        if (me.max < data[x][y]) {
          ...
          for (var one in data) for (var two in data[one]) heatmap.drawAlpha(one, two, data[one][two]);
    ...
    heatmap.prototype = {
      configure: function(config) {
        ...
        me.set('radiusIn', rin || 15),
          ...
          me.set(
            'element',
            config.element instanceof Object ? config.element : document.getElementById(config.element)
          );
...
CStudioAuthoring.Module.moduleLoaded('heatmap-support', h337);
