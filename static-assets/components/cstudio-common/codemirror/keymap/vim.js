...
(function () {
  ...
  function showAlert(cm, text) {
    var esc = text.replace(/[<&]/, function (ch) {
      return ch == '<' ? '&lt;' : '&amp;';
    });
  ...
  iterObj(
    {
      ...
      "'~'": function (cm) {
        var cur = cm.getCursor(),
          cHar = cm.getRange({ line: cur.line, ch: cur.ch }, { line: cur.line, ch: cur.ch + 1 });
  ...
  function setupPrefixBindingForKey(m) {
    ...
    CodeMirror.keyMap['vim-prefix-r'][m] = function (cm) {
      var cur = cm.getCursor();
      cm.replaceRange(toLetter(m), { line: cur.line, ch: cur.ch }, { line: cur.line, ch: cur.ch + 1 });
