(function () {
  ...
  function scriptHint(editor, keywords, getToken) {
    ...
    while (tprop.className == 'property') {
      ...
      if (tprop.string == ')') {
        ...
        tprop = getToken(editor, { line: cur.line, ch: tprop.start });
        if (tprop.className == 'variable') tprop.className = 'function';
        else return; // no clue
      }
  ...
  function getCompletions(token, context, keywords) {
    ...
    if (context) {
      ...
      var obj = context.pop(),
        base;
      if (obj.className == 'variable') base = window[obj.string];
      else if (obj.className == 'string') base = '';
      else if (obj.className == 'atom') base = 1;
        ...
        if (
          ...
          base = window.jQuery();
        else if (window._ != null && obj.string == '_' && typeof window._ == 'function') base = window._();
      }
