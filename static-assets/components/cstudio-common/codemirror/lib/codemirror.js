...
window.CodeMirror = (function () {
  ...
  function CodeMirror(place, givenOptions) {
    ...
    function onMouseDown(e) {
      ...
      for (var n = e_target(e); n != wrapper; n = n.parentNode) if (n.parentNode == sizer && n != mover) return;
    ...
    function updateLinesNoUndo(from, to, lines, selFrom, selTo) {
      ...
      if (options.onChange) {
        ...
        for (var i = 0; i < lines.length; ++i) if (typeof lines[i] != 'string') lines[i] = lines[i].text;
    ...
    function visibleLines(scrollTop) {
      ...
      return { from: lineAtHeight(doc, fromHeight), to: lineAtHeight(doc, toHeight) };
    }
      ...
      if (collapsed) {
        ...
        var add = function (left, top, right, height) {
          var rstyle = quirksMode
            ? 'width: ' + (!right ? clientWidth : clientWidth - right - left) + 'px'
            : 'right: ' + right + 'px';
          fragment.appendChild(
            elt(
              ...
              null,
              'CodeMirror-selected',
              'position: absolute; left: ' + left + 'px; top: ' + top + 'px; ' + rstyle + '; height: ' + height + 'px'
            )
          );
        };
    ...
    function themeChanged() {
      scroller.className =
        scroller.className.replace(/\s*cm-s-\S+/g, '') + options.theme.replace(/(^|\s)\s*/g, ' cm-s-');
    }
    ...
    function measureLine(line, ch) {
      ...
      var wbr =
        options.lineWrapping && ch < line.text.length && spanAffectsWrapping.test(line.text.slice(ch - 1, ch + 1));
  ...
  function hlSpans(val) {
    return typeof val == 'string' ? null : val.markedSpans;
  }
