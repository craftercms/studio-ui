CodeMirror.defineMode('css', function (config) {
  ...
  return {
    ...
    token: function (stream, state) {
      ...
      else if (context == '@media' && /\b(keyword|attribute)\b/.test(style)) state.stack.push('@mediaType');
    ...
    indent: function (state, textAfter) {
      ...
      if (/^\}/.test(textAfter)) n -= state.stack[state.stack.length - 1] == 'propertyValue' ? 2 : 1;
