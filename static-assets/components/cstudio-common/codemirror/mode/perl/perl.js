...
CodeMirror.defineMode('perl', function (config, parserConfig) {
  ...
  function tokenPerl(stream, state) {
    ...
    if (stream.match(/^\-?[\d\.]/, false))
      if (stream.match(/^(\-?(\d*\.\d+(e[+-]?\d+)?|\d+\.\d*)|0x[\da-fA-F]+|0b[01]+|\d+(e[+-]?\d+)?)/)) return 'number';
    if (stream.match(/^<<(?=\w)/)) {
      // NOTE: <<SOMETHING\n...\nSOMETHING\n
      stream.eatWhile(/\w/);
    ...
    if (ch == '/') {
      if (!/~\s*$/.test(stream.prefix())) return 'operator';
...
CodeMirror.StringStream.prototype.eatSuffix = function (c) {
  ...
  var y;
  if (x <= 0) this.pos = 0;
  else if (x >= (y = this.string.length - 1)) this.pos = y;
  else this.pos = x;
};
