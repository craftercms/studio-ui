        ...
        CrafterCMSNext.system.store.dispatch({
          ...
          payload: {
            ...
            items: items.map((item) =>
              CrafterCMSNext.util.content.parseLegacyItemToSandBoxItem(item)
            ),
            onSuccess: {
              type: 'BATCH_ACTIONS',
              payload: [
      ...
      approveCommon: function (site, items, approveType) {
        ...
        CrafterCMSNext.system.store.dispatch({
          ...
          payload: {
            open: true,
            items: items.map((item) =>
              CrafterCMSNext.util.content.parseLegacyItemToDetailedItem(item)
            ),
            scheduling,
            onSuccess: {
              type: 'BATCH_ACTIONS',
          ...
          if ($.fn.resizable) {
            $modal.find('.studio-ice-dialog').resizable({
              ...
              start: function (event, ui) {
                ...
                $('#in-context-edit-editor-' + editorId, topWindow.document).css(
                  'pointer-events',
                  'none'
                );
              },
