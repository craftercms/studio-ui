  ...
  CSA.register({
    ...
    Operations: {
      ...
      performSimpleIceEdit: function (item, field, isEdit, callback, aux) {
        ...
        var $modal = $(
          '<div><div class="no-ice-mask"></div><div class="studio-ice-dialog studio-ice-container-' +
            editorId +
        ...
        CSA.Env.Loader.use(controller, function () {
          CStudioAuthoring.Service.getInContextEditView({
            success: function (response) {
              ...
              var template =
                '<iframe id="in-context-edit-editor-' +
                editorId +
                '" frameborder="0" style="z-index:' +
                window.top.studioFormZorder +
                ';" onload="CStudioAuthoring.InContextEdit.autoSizeIceDialog();"></iframe>"';
        ...
        window.setTimeout(function () {
          $(function () {
            $modal.find('.studio-ice-dialog').resizable({
              ...
              start: function (event, ui) {
                $('#engineWindow', window.top.document).css('pointer-events', 'none');
                $('#in-context-edit-editor-' + editorId, window.top.document)
                  .css('pointer-events', 'none')
                  .height('');
              },
              stop: function (event, ui) {
        ...
        var CSA = CStudioAuthoring,
          ...
          doEdit = function () {
            if (uri.indexOf('/site') == 0) {
              CSA.Operations.openContentWebForm(formId, id, noderef, path, true, asPopup, callback, auxParams);
            } else if (
              uri.indexOf('.ftl') != -1 ||
              uri.indexOf('.css') != -1 ||
              uri.indexOf('.js') != -1 ||
    ...
    Service: {
      ...
      search: function (site, searchContext, callback) {
        if (!searchContext.firstTime) {
          ...
          searchContext.sortBy =
            !searchContext.sortBy || searchContext.sortBy == 'relevance' ? '' : searchContext.sortBy;
          ...
          searchConfig +=
            ...
            '",' +
            '"pageSize": "' +
            searchContext.pageSize +
            '",' +
            '"sortBy": "' +
            searchContext.sortBy +
            '",' +
            '"sortAscending": "' +
            searchContext.sortAscending +
            '",' +
            '"filters": [';
          if (searchContext.filters != null) {
            ...
            for (var i = 0; i < searchContext.filters.length; i++) {
              ...
              if (value != null && value != '' && value != 'all') {
                searchConfig +=
                  '{"qname" : "' + name + '" , "value" : "' + value + '", "useWildCard" : "' + useWildCard + '"}';
                seperatorFlag = true;
      ...
      getCMISContentByBrowser: function (site, repoId, path, callback) {
        var serviceUri =
          this.getCMISContentByBrowseUri + '?site_id=' + site + '&cmis_repo_id=' + repoId + '&path=' + path;
        serviceUri = serviceUri + '&nocache=' + new Date();
      ...
      getWebDAVContentByBrowser: function (site, profileId, path, callback, filter) {
        var serviceUri =
          this.getWebDAVContentByBrowseUri + '?site_id=' + site + '&profile=' + profileId + '&path=' + path;
...
if (window.top === window.self) {
  (function startAuthLoop() {
    if (typeof CStudioAuthoringContext != 'undefined') {
      ...
      CStudioAuthoring.Service.lookupConfigurtion(CStudioAuthoringContext.site, '/site-config.xml', authLoopCb);
    } else {
