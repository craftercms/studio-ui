  ...
  function (crafter, $, $ui, Animator, Communicator) {
    ...
    function enableDnD(components, initialComponentModel, browse) {
      ...
      function restrictions($dropZone, $component, isNew) {
        ...
        var DestContentType =
          $component
            .parents('[data-studio-zone-content-type]')
            .attr('data-studio-zone-content-type') || null;
        ...
        if (!isNew) {
          ...
          destTrackingNumber = $component
            .parents('[data-studio-zone-tracking]')
      ...
      function updateDop(self, me, ui) {
        ...
        var zone =
          $component
            .parents('[data-studio-components-target]')
            .attr('data-studio-components-target') || null;
        if (!isNew) {
          componentType = $component.attr('data-studio-embedded-item-id')
            ? 'embedded-content'
            : 'shared-content';
        } else {
          let path = $component.attr('data-studio-component-path');
        ...
        if (
          orgZoneComp.attr('data-studio-component-path') !=
            destZoneComp.attr('data-studio-component-path') ||
          (orgZoneComp.attr('data-studio-component-path') ==
            destZoneComp.attr('data-studio-component-path') &&
          ...
          (orgZoneComp.attr('data-studio-component-path') ==
            destZoneComp.attr('data-studio-component-path') &&
            orgZoneComp.attr('data-studio-tracking-number') ==
              destZoneComp.attr('data-studio-tracking-number') &&
            $dropZone.attr('data-studio-components-objectid') ==
              ui.item
                .parents('[data-studio-components-target]')
                .attr('data-studio-components-objectid'))
    ...
    function componentDropped($dropZone, $component, datasource) {
      var compPath = $dropZone
        .parents('[data-studio-component-path]')
        .attr('data-studio-component-path');
      var compTracking = $dropZone
        .parents('[data-studio-component-path]')
        .attr('data-studio-tracking-number');
      ...
      setTimeout(function () {
        $('[data-studio-components-target]').each(function () {
          ...
          if (
            ...
            if (
              compTracking ==
              $(this).parents('[data-studio-component-path]').attr('data-studio-tracking-number')
            ) {
              var $el = $(this),
                zoneName = $el.attr('data-studio-components-target');
              zones[zoneName] = [];
        ...
        if (!$el.parents('[data-studio-embedded-item-id]').attr('data-studio-embedded-item-id')) {
          if (name.indexOf('.') < 0) {
            ...
            if (objectId) {
              if (!found[id] || objectId == data['objectId']) {
                if (
                  (data[structure1][index][structure2] ||
                    data[structure1][index][structure2] == '') &&
    ...
    function renderPalette(components, browse) {
      ...
      $.each(components || [], function (i, category) {
        ...
        html.push(
          '<sh2 class="studio-category-name studio-category-name-collapse">' +
            category.label +
            '</sh2>'
        );
        html.push('<sul>');
