...
(function (angular) {
      ...
      function authLoop() {
        ...
        if (!window.reLoginModalOn && !isInIframe && 'login' !== $state.current.name) {
          ...
          authService.validateSession().then(
            ...
            function errorCallback(response) {
              if (response.status == 401 || response.status == 301 || response.status == 302 || response.status == 0) {
                if (authService.getUser().authenticationType == Constants.HEADERS) {
                  $state.go('login');
                } else {
                  $scope.reLoginModal = showReLoginModal();
  ...
  app.controller('SitesCtrl', [
    ...
    function ($scope, $state, $location, sitesService, authService, $modal, $cookies, $timeout, Constants) {
      ...
      $scope.sitesPag = {
        sitesPerPage: $cookies['crafterStudioSitesPagination']
          ? parseInt($cookies['crafterStudioSitesPagination'], 10)
          : 15
      };
