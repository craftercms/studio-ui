...
(function() {
	var JSONRequest = tinymce2.util.JSONRequest, each = tinymce2.each, DOM = tinymce2.DOM;

	tinymce2.create('tinymce2.plugins.SpellcheckerPlugin', {
		getInfo : function() {
			return {
				longname : 'Spellchecker',
				author : 'Moxiecode Systems AB',
				authorurl : 'http://tinymce2.moxiecode.com',
				infourl : 'http://wiki.moxiecode.com/index.php/tinymce2:Plugins/spellchecker',
		...
		setLanguage: function(lang) {
			...
			if (tinymce2.grep(t.languages, function(v) { return v === lang; }).length === 0) {
		...
		_getWords : function() {
			...
			if (ed.getParam('spellchecker_word_pattern')) {
				...
				tx = tinymce2.trim(tx.replace(/(\s+)/g, ' '));
				rawWords = tx.split(' ');
			}
	...
	// Register plugin
	tinymce2.PluginManager.add('spellchecker', tinymce2.plugins.SpellcheckerPlugin);
})();
