...
function aggregateStyles(allStyles) {
	...
	tinymce2.each(allStyles, function(style) {
		if (style !== '') {
			var parsedStyles = tinymce2Popup.editor.dom.parseStyle(style);
...
function init(ed) {
	...
	existingStyles = aggregateStyles(tinymce2Popup.getWindowArg('styles'));
	ce.style.cssText = tinymce2Popup.editor.dom.serializeStyle(existingStyles);
...
function setupFormData() {
	...
	f.text_color.value = tinymce2Popup.editor.dom.toHex(ce.style.color);
	...
	f.background_color.value = tinymce2Popup.editor.dom.toHex(ce.style.backgroundColor);
	...
	f.elements.border_color_top.value = tinymce2Popup.editor.dom.toHex(f.elements.border_color_top.value);
	f.elements.border_color_right.value = tinymce2Popup.editor.dom.toHex(f.elements.border_color_right.value);
	f.elements.border_color_bottom.value = tinymce2Popup.editor.dom.toHex(f.elements.border_color_bottom.value);
	f.elements.border_color_left.value = tinymce2Popup.editor.dom.toHex(f.elements.border_color_left.value);
...
function applyAction() {
	var ce = document.getElementById('container'), ed = tinymce2Popup.editor;

	generateCSS();

	tinymce2Popup.restoreSelection();

	var newStyles = tinymce2Popup.editor.dom.parseStyle(ce.style.cssText);
	...
	if (applyActionIsInsert) {
		...
		if (tinymce2Popup.getWindowArg('applyStyleToBlocks')) {
		...
		ed.dom.setAttrib(nodes, 'style', tinymce2Popup.editor.dom.serializeStyle(newStyles));
	}
}
	...
	for (i=0; i<f.length; i++) {
		for (a=0; a<f[i].elements.length; a++) {
			if (f[i].elements[a].disabled)
				tinymce2Popup.editor.dom.addClass(f[i].elements[a], "disabled");
			else
				tinymce2Popup.editor.dom.removeClass(f[i].elements[a], "disabled");
		}
	}
}
...
function fillSelect(f, s, param, dval, sep, em) {
	...
	ar = tinymce2Popup.getParam(param, dval).split(sep);
...
function updateTextDecorations(){
	...
	tinymce2.each(textDecorations, function(id) {
