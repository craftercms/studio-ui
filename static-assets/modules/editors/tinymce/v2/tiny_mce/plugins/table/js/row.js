...
function init() {
	...
	if (dom.select('td.mceSelected,th.mceSelected', trElm).length == 0) {
		...
		tinymce2Popup.dom.hide('action');
}

function updateAction() {
	var inst = tinymce2Popup.editor, dom = inst.dom, trElm, tableElm, formObj = document.forms[0];
	...
	if (dom.select('td.mceSelected,th.mceSelected', trElm).length > 0) {
		tinymce2.each(tableElm.rows, function(tr) {
			var i;
