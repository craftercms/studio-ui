...
(function() {
	var rootAttributes = tinymce2.explode('id,name,width,height,style,align,class,hspace,vspace,bgcolor,type'), excludedAttrs = tinymce2.makeMap(rootAttributes.join(',')), Node = tinymce2.html.Node,
		mediaTypes, scriptRegExp, JSON = tinymce2.util.JSON, mimeTypes;
	...
	tinymce2.create('tinymce2.plugins.MediaPlugin', {
		init : function(ed, url) {
			...
			for (i = 0; i < mediaTypes.length; i++) {
				...
				item = {
					name : name,
					clsids : tinymce2.explode(mediaTypes[i][1] || ''),
					mimes : tinymce2.explode(mediaTypes[i][2] || ''),
			...
			tinymce2.each(ed.getParam("media_types",
				...
				item = item.split(/=/);
				extensions = tinymce2.explode(item[1].toLowerCase());
			...
			ed.addCommand('mceMedia', function() {
				...
				if (isMediaImg(img)) {
					...
					if (data) {
						...
						tinymce2.each(rootAttributes, function(name) {
		...
		getInfo : function() {
			return {
				...
				infourl : 'http://wiki.moxiecode.com/index.php/tinymce2:Plugins/media',
		...
		/**
		 * Converts a tinymce2.html.Node image element to video/object/embed.
		 */
		imgToObject : function(node, args) {
			...
			function addPlayer(video_src, poster_src) {
				...
				if (flashPlayer) {
					...
					tinymce2.each(flashVars, function(value, name) {
					...
					tinymce2.each(params, function(value, name) {
			...
			if (this.editor.settings.media_use_script) {
				...
				value.value = 'write' + typeItem.name + '(' + JSON.serialize(tinymce2.extend(data.params, {
			...
			if (typeItem.name === 'Video' && data.video.sources[0]) {
				...
				video = new Node('video', 1).attr(tinymce2.extend({
					id : node.attr('id'),
			...
			if (typeItem.name === 'Audio' && data.video.sources[0]) {
				// Create new object element
				audio = new Node('audio', 1).attr(tinymce2.extend({
					id : node.attr('id'),
					width: normalizeSize(node.attr('width')),
					height: normalizeSize(node.attr('height')),
					style : style
		...
		/**
		 * Converts a tinymce2.html.Node video/object/embed to an img element.
		 *
		...
		objectToImg : function(node) {
			...
			function getInnerHTML(node) {
				return new tinymce2.html.Serializer({
					inner: true,
					validate: false
				}).serialize(node);
			};
	...
	// Register plugin
	tinymce2.PluginManager.add('media', tinymce2.plugins.MediaPlugin);
})();
