(function() {
	...
	if (url = tinymce2Popup.getParam("media_external_list_url"))
	...
	window.Media = {
		init : function() {
			...
			self.editor = editor = tinymce2Popup.editor;
			...
			self.data = clone(tinymce2Popup.getWindowArg('data'));
			self.dataToForm();
			self.preview();
		...
		insert : function() {
			var editor = tinymce2Popup.editor;
		...
		moveStates : function(to_form, field) {
			...
			function parseQueryParams(str) {
				...
				if (str) {
					tinymce2.each(str.split('&'), function(item) {
			...
			if (to_form) {
				if (data.type == 'video') {
					...
					// Check flash vars
					if (data.type == 'flash') {
						tinymce2.each(editor.getParam('flash_video_player_flashvars', {url : '$url', poster : '$poster'}), function(value, name) {
							if (value == '$url')
		...
		getMediaListHTML : function() {
			if (typeof(tinymce2MediaList) != "undefined" && tinymce2MediaList.length > 0) {
				var html = "";
				...
				for (var i=0; i<tinymce2MediaList.length; i++)
					html += '<option value="' + tinymce2MediaList[i][1] + '">' + tinymce2MediaList[i][0] + '</option>';
		...
		getMediaTypeHTML : function(editor) {
			function option(media_type, element) {
				...
				return '<option value="'+media_type+'">'+tinymce2Popup.editor.translate("media_dlg."+media_type)+'</option>'
			}
		...
		setDefaultDialogSettings : function(editor) {
			...
			tinymce2.each(defaultDialogSettings, function(v, k) {
				setVal(k, v);
			});
		}
	};
