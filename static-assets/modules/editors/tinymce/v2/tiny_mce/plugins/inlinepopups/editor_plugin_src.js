...
(function() {
	var DOM = tinymce2.DOM, Element = tinymce2.dom.Element, Event = tinymce2.dom.Event, each = tinymce2.each, is = tinymce2.is;

	tinymce2.create('tinymce2.plugins.InlinePopups', {
		init : function(ed, url) {
			// Replace window manager
			ed.onBeforeRenderUI.add(function() {
				ed.windowManager = new tinymce2.InlineWindowManager(ed);
		...
		getInfo : function() {
			return {
				...
				infourl : 'http://wiki.moxiecode.com/index.php/tinymce2:Plugins/inlinepopups',
				version : tinymce2.majorVersion + "." + tinymce2.minorVersion
			};
		}
	});

	tinymce2.create('tinymce2.InlineWindowManager:tinymce2.WindowManager', {
		...
		open : function(f, p) {
			...
			f.height = parseInt(f.height || 240) + (tinymce2.isIE ? 8 : 0);
...
//			f.top -= po.y;
			...
			t._addAll(DOM.doc.body,
				['div', {id : id, role : 'dialog', 'aria-labelledby': f.type ? id + '_content' : id + '_title', 'class' : (ed.settings.inlinepopups_skin || 'clearlooks2') + (tinymce2.isIE && window.getSelection ? ' ie9' : ''), style : 'width:100px;height:100px'},
			...
			if (u) {
				if (tinymce2.relaxedDomain)
					u += (u.indexOf('?') == -1 ? '?' : '&') + 'mce_rdomain=' + tinymce2.relaxedDomain;
			...
			if (tinymce2.isIE6 || /Firefox\/2\./.test(navigator.userAgent) || (tinymce2.isIE && !DOM.boxModel))
		...
		_addAll : function(te, ne) {
			var i, n, t = this, dom = tinymce2.DOM;
...
//			cp = DOM.getPos(t.editor.getContainer());
			...
			function startMove() {
				...
				if (tinymce2.isIE6 || (tinymce2.isIE && !DOM.boxModel))
	...
	// Register plugin
	tinymce2.PluginManager.add('inlinepopups', tinymce2.plugins.InlinePopups);
})();
