...
function initCommonAttributes(elm) {
	var formObj = document.forms[0], dom = tinymce2Popup.editor.dom;
...
function setAttrib(elm, attrib, value) {
	...
	tinymce2Popup.editor.dom.setAttrib(elm, attrib, value || valueElm.value);
}
...
SXE.initElementDialog = function(element_name) {
	...
	document.forms[0].insert.value = tinymce2Popup.getLang(SXE.currentAction, 'Insert', true); 
}
...
SXE.insertElement = function(element_name) {
	...
	if (elm == null) {
		...
		if(s.length > 0) {
			...
			var elementArray = tinymce2.grep(SXE.inst.dom.select(element_name));
...
SXE.removeElement = function(element_name){
	...
	if(elm && elm.nodeName.toUpperCase() == element_name.toUpperCase()){
		tinymce2.execCommand('mceRemoveNode', false, elm);
		SXE.inst.nodeChanged();
		tinymce2Popup.execCommand('mceEndUndoLevel');
	}
}
...
function insertInlineElement(en) {
	...
	tinymce2.each(dom.select('span,font'), function(n) {
