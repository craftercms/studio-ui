...
(function() {
	...
	tinymce2.create('tinymce2.plugins.FullScreenPlugin', {
		init : function(ed, url) {
			...
			ed.addCommand('mceFullScreen', function() {
				...
				if (ed.getParam('fullscreen_is_enabled')) {
					if (ed.getParam('fullscreen_new_window'))
						...
						DOM.win.setTimeout(function() {
							...
							var originalEditor = tinymce2.get(fullscreenEditor.getParam('fullscreen_editor_id'));
							originalEditor.plugins.fullscreen.saveState(fullscreenEditor);

							tinymce2.remove(fullscreenEditor);
						}, 10);
					}
				...
				if (ed.getParam('fullscreen_new_window')) {
					...
					if (tinymce2.isOpera && fullscreen_overflow == 'visible')
						fullscreen_overflow = 'auto';

					// Fixes an IE bug where horizontal scrollbars would appear
					if (tinymce2.isIE && fullscreen_overflow == 'scroll')
						fullscreen_overflow = 'auto';

					// Fixes an IE bug where the scrollbars doesn't reappear
					if (tinymce2.isIE && (fullscreen_html_overflow == 'visible' || fullscreen_html_overflow == 'scroll'))
					...
					if (tinymce2.isIE6 || document.compatMode == 'BackCompat')
					...
					tinymce2.each(ed.settings, function(v, n) {
						s[n] = v;
					});
					...
					s.save_onsavecallback = function() {
						ed.setContent(tinymce2.get(s.id).getContent());
						ed.execCommand('mceSave');
					};

					tinymce2.each(ed.getParam('fullscreen_settings'), function(v, k) {
						s[k] = v;
					});
					...
					tinymce2.oldSettings = tinymce2.settings; // Store old settings, the Editor constructor overwrites them
					t.fullscreenEditor = new tinymce2.Editor('mce_fullscreen', s);
					...
					t.fullscreenElement = new tinymce2.dom.Element('mce_fullscreen_container');
					t.fullscreenElement.update();
					//document.body.overflow = 'hidden';

					t.resizeFunc = tinymce2.dom.Event.add(DOM.win, 'resize', function() {
						var vp = tinymce2.DOM.getViewPort(), fed = t.fullscreenEditor, outerSize, innerSize;
			...
			t.saveState = function(fullscreenEditor) {
				...
				if (!ed.getParam('fullscreen_new_window')) {
					tinymce2.dom.Event.remove(DOM.win, 'resize', t.resizeFunc);
					delete t.resizeFunc;
				...
				tinymce2.settings = tinymce2.oldSettings; // Restore old settings

				// clear variables
				delete tinymce2.oldSettings;
		...
		getInfo : function() {
			return {
				...
				infourl : 'http://wiki.moxiecode.com/index.php/tinymce2:Plugins/fullscreen',
	...
	// Register plugin
	tinymce2.PluginManager.add('fullscreen', tinymce2.plugins.FullScreenPlugin);
})();
