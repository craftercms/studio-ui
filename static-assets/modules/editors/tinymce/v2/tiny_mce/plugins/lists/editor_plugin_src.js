...
(function() {
	var each = tinymce2.each, Event = tinymce2.dom.Event, bookmark;
	...
	function hasParentInList(ed, e, list) {
		return ed.dom.getParent(e, function(p) {
			return tinymce2.inArray(list, p) !== -1;
		});
	}
	...
	tinymce2.create('tinymce2.plugins.Lists', {
		init: function(ed) {
			...
			function isTabInList(e) {
				...
				return e.keyCode === tinymce2.VK.TAB && !(e.altKey || e.ctrlKey) &&
			...
			function isEmptyIE9Li(li) {
				...
				var lis = tinymce2.grep(li.parentNode.childNodes, function(n) {return n.tagName == 'LI'});
				var isLastLi = li == lis[lis.length - 1];
				var child = li.firstChild;
				return tinymce2.isIE9 && isLastLi && (child.nodeValue == String.fromCharCode(160) || child.nodeValue == String.fromCharCode(32));
			}

			function isEnter(e) {
				return e.keyCode === tinymce2.VK.ENTER;
			}
			...
			function cancelDefaultEvents(ed, e) {
				...
				if (state == LIST_TABBING || state == LIST_EMPTY_ITEM || tinymce2.isGecko && state == LIST_ESCAPE) {
					Event.cancel(e);
				}
			}
			...
			function isCursorAtEndOfContainer() {
				...
				if (startContainer.nodeType == 3) {
					...
					if (tinymce2.isIE9 && value.length > 1 && value.charCodeAt(value.length-1) == 32) {
			...
			function createNewLi(ed, e) {
				if (isEnterWithoutShift(e) && isEndOfListItem()) {
					...
					if (tinymce2.isIE6 || tinymce2.isIE7 || tinymce2.isIE8) {
			...
			function imageJoiningListItem(ed, e) {
				...
				var n = ed.selection.getStart();
				if (e.keyCode != tinymce2.VK.BACKSPACE || n.tagName !== 'IMG')
					return;
			...
			ed.onKeyUp.add(function(ed, e) {
				if (state == LIST_TABBING) {
					...
					} else if (tinymce2.isIE9 || tinymce2.isGecko) {
			...
			function fixDeletingFirstCharOfList(ed, e) {
				function listElements(li) {
					var elements = [];
					var walker = new tinymce2.dom.TreeWalker(li.firstChild, li);
				...
				if (e.keyCode == tinymce2.VK.BACKSPACE) {
					var li = getLi();
					if (li) {
						var list = ed.dom.getParent(li, 'ol,ul'),
			...
			function fixDeletingEmptyLiInWebkit(ed, e) {
				var li = getLi();
				if (e.keyCode === tinymce2.VK.BACKSPACE && ed.dom.is(li, 'li') && li.parentNode.firstChild!==li) {
					if (ed.dom.select('ul,ol', li).length === 1) {
						var prevLi = li.previousSibling;
						ed.dom.remove(ed.dom.select('br', li));
						ed.dom.remove(li, true);
						var textNodes = tinymce2.grep(prevLi.childNodes, function(n){ return n.nodeType === 3 });
			...
			if (tinymce2.isGecko || tinymce2.isWebKit) {
		...
		applyList: function(targetListType, oppositeListType) {
			...
			function changeList(element) {
				if (tinymce2.inArray(applied, element) !== -1) {
					return;
				}
			...
			function convertListItemToParagraph(element) {
				...
				if (mergedElement === element) {
					...
					// Manually iterate to ensure we handle modifications correctly (doesn't work with tinymce2.each)
		...
		outdent: function(ui, elements) {
			...
			var listElements = elements && tinymce2.is(elements, 'array') ? elements : this.selectedBlocks();
			this.process({
				'LI': outdentLI,
		...
		process: function(actions) {
			...
			function isEmptyElement(element) {
				var excludeBrsAndBookmarks = tinymce2.grep(element.childNodes, function(n) {
		...
		getInfo: function() {
			return {
				...
				infourl : 'http://wiki.moxiecode.com/index.php/tinymce2:Plugins/lists',
				version : tinymce2.majorVersion + "." + tinymce2.minorVersion
			};
		}
	});
	tinymce2.PluginManager.add("lists", tinymce2.plugins.Lists);
}());
