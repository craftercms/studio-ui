...
(function() {
	var each = tinymce2.each, Node = tinymce2.html.Node;

	tinymce2.create('tinymce2.plugins.FullPagePlugin', {
		...
		getInfo : function() {
			return {
				...
				infourl : 'http://wiki.moxiecode.com/index.php/tinymce2:Plugins/fullpage',
		...
		_dataToHtml : function(data) {
			...
			html = new tinymce2.html.Serializer({
				validate: false,
				indent: true,
		...
		_parseHeader : function() {
			...
			return new tinymce2.html.DomParser({
				validate: false,
				root_name: '#document'
			}).parse(this.head);
		},
		...
		_getContent : function(ed, o) {
			...
			if (!o.source_view || !ed.getParam('fullpage_hide_in_source_view'))
				o.content = tinymce2.trim(self.head) + '\n' + tinymce2.trim(o.content) + '\n' + tinymce2.trim(self.foot);
		}
	});

	// Register plugin
	tinymce2.PluginManager.add('fullpage', tinymce2.plugins.FullPagePlugin);
})();
