...
(function(tinymce2) {
	var DOM = tinymce2.DOM, Event = tinymce2.dom.Event, extend = tinymce2.extend, each = tinymce2.each, Cookie = tinymce2.util.Cookie, lastExtID, explode = tinymce2.explode;
	...
	tinymce2.ThemeManager.requireLangPack('advanced');

	tinymce2.create('tinymce2.themes.AdvancedTheme', {
		...
		init : function(ed, url) {
			...
			t.editor = ed;
			t.url = url;
			t.onResolveName = new tinymce2.util.Dispatcher(this);
			s = ed.settings;
			...
			if (tinymce2.is(s.theme_advanced_font_sizes, 'string')) {
				s.font_size_style_values = tinymce2.explode(s.font_size_style_values);
				s.font_size_classes = tinymce2.explode(s.font_size_classes || '');
		...
		_createStyleSelect : function(n) {
			...
			ctrl = ctrlMan.createListBox('styleselect', {
				...
				onselect : function(name) {
					...
					tinymce2.each(matches, function(match) {
						if (!name || match == name) {
							if (match)
...
*/
			...
			if (!ed.getParam('accessibility_focus'))
				Event.add(DOM.add(p, 'a', {href : '#'}, '<!-- IE -->'), 'focus', function() {tinymce2.get(ed.id).focus();});
		...
		_addToolbars : function(c, o) {
			...
			h.push(DOM.createHTML('a', {href : '#', accesskey : 'z', title : ed.getLang("advanced.toolbar_focus"), onfocus : 'tinymce2.getInstanceById(\'' + ed.id + '\').focus();'}, '<!-- IE -->'));
		...
		_nodeChanged : function(ed, cm, n, co, ob) {
			...
			tinymce2.each(t.stateControls, function(c) {
			...
			if (c = cm.get('styleselect')) {
				...
				matches = ed.formatter.matchAll(formatNames);
				c.select(matches[0]);
				tinymce2.each(matches, function(match, index) {
					if (index > 0) {
						c.mark(match);
					}
				});
			...
			if (s.theme_advanced_path && s.theme_advanced_statusbar_location) {
				...
				getParent(function(n) {
					...
					// Handle prefix
					if (tinymce2.isIE && n.scopeName !== 'HTML' && n.scopeName)
						na = n.scopeName + ':' + na;
					...
					//u = "javascript:tinymce2.EditorManager.get('" + ed.id + "').theme._sel('" + (de++) + "');";
				...
				if (DOM.select('a', p).length > 0) {
					t.statusKeyboardNavigation = new tinymce2.ui.KeyboardNavigation({
	...
	tinymce2.ThemeManager.add('advanced', tinymce2.themes.AdvancedTheme);
}(tinymce2));
